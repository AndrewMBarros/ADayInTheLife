...
TELEGRAM_CHAT_ID = os.environ.get("TELEGRAM_CHAT_ID", "-1004490733318")
...
def enviar_mensagem_telegram(texto):
    ...
    if not TELEGRAM_TOKEN or TELEGRAM_TOKEN == "SEU_TOKEN_AQUI":
        raise RuntimeError(
            "TELEGRAM_TOKEN não está configurado (veio vazio ou com o valor "
            "padrão). Confira o Secret TELEGRAM_TOKEN no GitHub Actions."
        )
    if not TELEGRAM_CHAT_ID or TELEGRAM_CHAT_ID == "SEU_CHAT_ID_AQUI":
        raise RuntimeError(
            "TELEGRAM_CHAT_ID não está configurado (veio vazio ou com o valor "
            "padrão). Confira o Secret TELEGRAM_CHAT_ID no GitHub Actions."
        )
    ...
    if resp.status_code != 200:
        # Isso força o workflow a falhar (ficar vermelho) e mostra
        # a resposta completa da API do Telegram no log do GitHub Actions.
        raise RuntimeError(
            f"Erro ao enviar mensagem para o Telegram: "
...
def main():
    ...
    if not noticias:
        print(
            "ATENÇÃO: 0 notícias encontradas. Isso pode indicar que o site "
            "bloqueou a requisição (comum com IPs de datacenter/GitHub "
            "Actions) ou que a classe 'aday' mudou no HTML."
        )
